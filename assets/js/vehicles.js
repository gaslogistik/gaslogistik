/* ============================================================
VEHICLES.JS — v3 (2026-08-07) — UTWARDZONA
🔧 v3 FIX:
 • timeout obejmuje TERAZ całe pobranie (nagłówek + treść) — koniec
   z wiszącym "pending" i wiecznym spinnerem,
 • prefetch ograniczony przez Promise.race (max 20 s),
 • WATCHDOG: spinner CHOWANY zawsze po max 60 s, cokolwiek się dzieje,
 • sekcje renderowane niezależnie (safeSection) — błąd jednej nie
   zabija strony,
 • baner offline pokazuje POWÓD błędu na ekranie,
 • reszta jak v2: natychmiastowy render z localStorage (SWR), pigułka
   statusu, auto-odświeżanie, tolerancyjne klucze (vGet).
============================================================ */
const API_URL =
...
HARD_RETRIES: 3,
PREFETCH_RACE_MS: 20000,
WATCHDOG_MS: 60000,
AUTO_REFRESH_MS: 5 * 60 * 1000,
MIN_REFETCH_GAP_MS: 45 * 1000,
MAX_AUTO_RETRY: 4
};
let lastGoodTs = null;
let lastFetchTs = 0;
...
/* ============================================================
🔧 v3 FETCH — timeout pilnuje CAŁEJ odpowiedzi (nagłówek + treść)
============================================================ */
...
// 🔧 v3: timer NIE jest kasowany przed przeczytaniem treści!
const text = await res.text();
clearTimeout(timer);
if (!res.ok) throw new Error("HTTP " + res.status);
if (!/^\s*[\[{]/.test(text)) {
throw new Error("Odpowiedź nie jest JSON-em: " + text.slice(0, 60));
}
...
/* ---------- BANER OFFLINE (z POWODEM na ekranie) ---------- */
...
msg.id = "vehicles-retry-banner-msg";
const btn = document.createElement("button");
btn.textContent = "Spróbuj teraz";
btn.style.cssText =
"background:#ff2a6d;color:#fff;border:none;border-radius:8px;" +
"padding:6px 12px;font-weight:700;cursor:pointer;flex:none;";
btn.addEventListener("click", () => { hardFailAutoRetries = 0; hardLoad(null); });
b.appendChild(msg);
b.appendChild(btn);
document.body.appendChild(b);
}
const msg = document.getElementById("vehicles-retry-banner-msg");
if (msg) {
msg.textContent = "Brak połączenia z API" +
(lastErrorText ? " (" + lastErrorText + ")" : "") + " — ponawianie…";
}
...
/* ---------- WATCHDOG — spinner NIGDY nie kręci się w nieskończoność ---------- */
function armSpinnerWatchdog() {
if (spinnerWatchdog) return;
spinnerWatchdog = setTimeout(() => {
console.warn("WATCHDOG: awaryjne ukrycie spinnera po " + CFG.WATCHDOG_MS + " ms");
hideSpinner();
}, CFG.WATCHDOG_MS);
}
function disarmSpinnerWatchdog() {
if (spinnerWatchdog) { clearTimeout(spinnerWatchdog); spinnerWatchdog = null; }
}
/* ---------- STYLE PODŚWIETLANIA ---------- */
...
/* ---------- PREFETCH (start przy parsowaniu) ---------- */
...
/* 🔧 v3: sekcje niezależne — błąd jednej nie zabija reszty */
function safeSection(name, fn) {
try { fn(); } catch (e) { console.error("SEKCJA " + name + ":", e); }
}
...
safeSection("counters",  () => updateCounters(trucks, tanktrailers));
safeSection("adrStatus", () => updateADRStatus(trucks, tanktrailers));
safeSection("extra",     () => updateADRExtraCounters(trucks, tanktrailers));
safeSection("reminder",  () => renderADRReminderCenter(trucks, tanktrailers));
safeSection("grids",     () => renderADRFleetGrids(trucks, tanktrailers));
safeSection("all",       () => renderAllVehicles(trucks, tanktrailers));
safeSection("search",    () => initAllVehiclesSearch(trucks, tanktrailers));
}
/* ---------- TWARDE ŁADOWANIE (gdy brak cache) ---------- */
...
try {
let data;
if (attempt === 1 && firstPromise) {
// 🔧 v3: prefetch max 20 s — nigdy nie blokujemy się na wiszącym obiecie
data = await Promise.race([
firstPromise,
sleep(CFG.PREFETCH_RACE_MS).then(() => {
throw new Error("prefetch timeout");
})
]);
} else {
...
data = await Promise.race([
existingPromise,
sleep(CFG.BG_TIMEOUT_MS).then(() => null)
]);
} catch (e) { data = null; }
}
if (!data) {
...
setTileValue("vc-adr-expiring", countADRExpiring(trucks, tanktrailers, 90));
setTileValue("vc-adr-expired", countADRExpired(trucks, tanktrailers));
}
...
if (docMissingEl) docMissingEl.textContent = missingVehicles.length;
...
docMissingListEl.textContent = missingVehicles
...
let badgeClass = "";
let remainingText = "";
if (type === "EXPIRED") { badgeClass = "badge-expired"; remainingText = "Expired"; }
else if (type === "7 DAYS") { badgeClass = "badge-7"; remainingText = `${diff} days`; }
else if (type === "21 DAYS") { badgeClass = "badge-21"; remainingText = `${diff} days`; }
reminders.push({ type, badgeClass, plates, internal, model, adrDate, remainingText });
}
[...trucks, ...tanktrailers].forEach((item) => {
const adr = getAdrRaw(item);
if (!adr) return;
...
let statusClass = "";
if (!adrDate) {
badgeClass = "adr-badge-critical"; badgeText = "NO ADR";
statusText = "DOCUMENT MISSING"; statusClass = "adr-status-critical";
} else {
const diff = daysBetween(today, adrDate);
if (diff < 0) {
badgeClass = "adr-badge-critical"; badgeText = "EXPIRED";
statusText = "ADR EXPIRED"; statusClass = "adr-status-critical";
} else if (diff <= 7) {
badgeClass = "adr-badge-days"; badgeText = `${diff} DAYS`;
statusText = "URGENT"; statusClass = "adr-status-critical";
} else if (diff <= 21) {
badgeClass = "adr-badge-days"; badgeText = `${diff} DAYS`;
statusText = "PLANNING REQUIRED"; statusClass = "adr-status-warning";
} else {
badgeClass = "adr-badge-days"; badgeText = `${diff} DAYS`;
statusText = "OK"; statusClass = "";
}
}
...
if (!row) { console.warn("Nie znaleziono wiersza:", selector); return; }
...
adrSearchIndex.push({
type: "TRUCK",
plates: getPlates(v), internal: getInternal(v), model: getModel(v),
});
});
...
function clearHighlight() {
document.querySelectorAll(".adr-vehicle-card.adr-card-highlight")
.forEach((card) => card.classList.remove("adr-card-highlight"));
}
function scrollToCard(match) {
clearHighlight();
const selector = `.adr-vehicle-card[data-plate="${CSS.escape(match.plates)}"][data-internal="${CSS.escape(match.internal)}"]`;
const card = document.querySelector(selector);
if (!card) { console.warn("Nie znaleziono karty:", selector); return; }
...
function loadVehiclesData() { bootstrapVehicles(); }
const VehiclesModule = {
loadVehiclesData, bootstrapVehicles, updateCounters, updateADRStatus,
updateADRExtraCounters, renderADRReminderCenter, renderADRFleetGrids,
countADRExpiring, countADRExpired, normalizeDate, daysBetween,
parseDate, fmtDate, showErrorState,
};
try { window.VehiclesModule = VehiclesModule; } catch (e) { }